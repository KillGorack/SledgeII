...
func apply_hit_force(force: float, target: Object = null, direction: Vector3 = Vector3.ZERO) -> void:
	...
	if force_target != null and force_target is RigidBody3D:
		force_target.apply_central_impulse(force * force_direction)
...
# NOTE: hit VFX (explosion/bullet hole/crit decal) only plays on the server
# right now, since this whole script only runs on the server. Other clients
# will see the projectile disappear (that part replicates) but won't see the
# effect itself yet - broadcasting these via RPC is a good next step.
func create_bullet_hole() -> void:
	if bullet_hole_one_off:
		return
	if weapon_settings.bullet_hole_prefab and collided_layer == 2: # world
