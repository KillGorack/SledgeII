...
func _load_map() -> void:
	...
	if target_file_id > 0:
		...
		# res://secret_level.tscn already exist" can't tell maps apart -
		# only re-download/re-mount when the requested map actually changed.
		if NetworkManager.mounted_map_file_id != target_file_id:
			await _download_and_mount_map(target_file_id)
	...
	# map's already-cached scene instead of the new one.
